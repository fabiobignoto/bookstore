...
class TestCategorytSerializer(APITestCase):
    def test_order_serializer_creates_valid_order(self):
        user = User.objects.create_user(
            username="my-username", password="my-password", email="my-email@server.com"
        )
        ...
        serializer = OrderSerializer(data={"products_id": products_id, "user": user.id})
        ...
        self.assertIn(product_one, order.product.all(), msg="Expected order not found")

        self.assertNotIn(
            product_two, order.product.all(), msg="Expected order not found"
        )

        self.assertIn(
            product_three, order.product.all(), msg="Expected order not found"
        )
