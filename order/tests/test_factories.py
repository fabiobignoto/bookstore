...
class OrderFactoryTest(TestCase):
    def test_order_factory_creates_fields(self):
        ...
        self.assertTrue(order.product, msg="Order's Product is equivalent to False.")
    ...
    def test_order_factory_add_products(self):
        ...
        self.assertEqual(
            2, len(product_list), msg="Wrong number of products found in the order."
        )
...
class UserFactoryTest(TestCase):
    def test_user_factory_creates_fields(self):
        user = UserFactory()
        self.assertTrue(user.username, msg="User's Username is equivalent to False")
